...
where $t$ is the hopping amplitude, $U$ is the onsite interaction, $\Delta$ is the pairing amplitude and $\mu$ is the chemical potential. To bring this into a more convenient form we gauge transform even site operators such that

\begin{align}
	a_i \to ia_i \quad \mathrm{and} \quad a_i^\dagger \to -ia_i^\dagger   .
\end{align}
...
\begin{align}
	H
	= - i\sum_{\langle i,j\rangle,\sigma}
	\left(
	t\left( a_{i,\sigma}^\dagger a_{j,\sigma} - a_{j,\sigma}^\dagger a_{i,\sigma}  \right)
	- \Delta \left(a_{i,\sigma}^\dagger a_{j,\sigma}^\dagger - a_{j,\sigma} a_{i,\sigma} \right)
	\right)
	- \mu \sum_{x,\sigma} n_{x,\sigma}
	+ U \sum_x
	\left(n_{x,\uparrow}-\frac{1}{2}\right)
...
Now we split each fermion into two Majroana fermions:
...
\begin{align}
	H = -\frac{i}{2}\sum_{<i,j>,\sigma}^{ } \left[\left(t - \Delta\right)  \left(\eta_{i,\sigma}\eta_{j,\sigma} \right)  + \left(t + \Delta\right)  \left(\gamma_{i,\sigma}\gamma_{j,\sigma} \right) \right] - i\frac{\mu}{2}\sum_{i,\sigma}^{ }\eta_{i,\sigma}\gamma_{i,\sigma} - \frac{U}{4}\sum_{i}^{ } \eta_{i,\uparrow}\gamma_{i,\uparrow}\eta_{i,\downarrow}\gamma_{i,\downarrow} 
\end{align}
...
\begin{align}
		H = \underbrace{-\frac{i}{2}\sum_{<i,j>,\sigma}^{ } \left[\left(t - \Delta\right)  \left(\eta_{i,\sigma}\eta_{j,\sigma} \right)  + \left(t + \Delta\right)  \left(\gamma_{i,\sigma}\gamma_{j,\sigma} \right) \right] - i\frac{\mu}{2}\sum_{i,\sigma}^{ }\eta_{i,\sigma}\gamma_{i,\sigma}}_{:= H_0} \underbrace{\mp \frac{U}{8}\sum_{i}^{ } \left(\eta_{i,\uparrow}\gamma_{i,\uparrow} \pm \eta_{i,\downarrow}\gamma_{i,\downarrow} \right)^2.}_{:=H_1}
\end{align}
...
\begin{align}
	 ...
	 \begin{array}{*{5}{c}|*{5}{c}}
	 	0 & \phantom{-}\Delta^{-} &  &  &  & \mu &  &  &  &  \\
	 	-\Delta^{-} & 0 & \phantom{-}\Delta^{-} &  &  &  & \mu &  &  &  \\
	 	& -\Delta^{-} & 0 & \ddots &  &  &  & \ddots &  &  \\
	 	&  & \ddots & \ddots & \phantom{-}\Delta^{-} &  &  &  & \ddots &  \\
	 	&  &  & -\Delta^{-} & 0 &  &  &  &  & \mu \\
	 	\hline
	 	-\mu &  &  &  &  & 0 & \phantom{-}\Delta^{+} &  &  &  \\
	 	& -\mu &  &  &  & -\Delta^{+} & 0 & \phantom{-}\Delta^{+} &  &  \\
	 	&  & \ddots &  &  &  & -\Delta^{+} & 0 & \ddots &  \\
	 	&  &  & \ddots &  &  &  & \ddots & \ddots & \phantom{-}\Delta^{+} \\
	 	&  &  &  & -\mu &  &  &  & -\Delta^{+} & 0
	 \end{array}
	 \right]},
\end{align}
...
where we set $\Delta^{\mp} = \left(t \mp \Delta \right)$. We notice that both the non-interaction Hamiltonian as well as the interaction part are block diagonal. The coupling between the two different spin parts has been removed. Therefore as shown in the appendix, the partition function can be rewritten as
...
\section{Applying the Pfaffian Formualtion to the Hubbard Model with a super conducting term}
	...
	\begin{align}
		...
		\end{bmatrix}
		 ...
		 \mathrm{Pf} \left(
		 \begin{bmatrix}
		 	\sqrt 2 \mathrm{sinh}\left(\frac{{\boldsymbol{B}^{(t)} }}{4}\right) & -\mathbb{I}\\
		 	\mathbb{I} &  \sqrt 2 \mathrm{sinh}\left(\frac{{\boldsymbol{B}^{(t)} }}{4}\right)
		 \end{bmatrix}
		 \right) \nonumber \\
		 &\quad\quad \times 
		 \mathrm{Pf} \left(
		 	\begin{bmatrix}
		 		\mathrm{tanh}\left(\frac{{\boldsymbol{A} }}{2} \right)    & -\mathbb{I} & \mathbb{I} & -\mathbb{I} & \dots &  -\mathbb{I} \\
		 		\mathbb{I} & \mathrm{tanh}\left(\frac{{\boldsymbol{B}^{(1)}} }{2} \right) & -\mathbb{I} & \mathbb{I} &   &  \vdots \\
		 		-\mathbb{I} & \mathbb{I} & \mathrm{tanh}\left(\frac{{\boldsymbol{A}} }{2} \right) & -\mathbb{I} &   & \vdots \\
		 		\mathbb{I} & -\mathbb{I} & \mathbb{I} & \mathrm{tanh}\left(\frac{{\boldsymbol{B}^{(2)}} }{2} \right) &   &  \vdots \\
		 		\vdots &   &   &   & \ddots & \vdots \\
		 		\mathbb{I} & \dots  & \dots  & \dots  & \dots & \mathrm{tanh}\left(\frac{{\boldsymbol{B}^{(N_\tau)}}}{2} \right) \\
		 	\end{bmatrix}
		 \right),
	\end{align}
	...
	\begin{align}
		...
		\begin{bmatrix}
			\mathrm{tanh}\left(\frac{{\boldsymbol{A}_{\eta}  }}{2} \right)    & -\mathbb{I} &  \dots &  -\mathbb{I} \\
			\mathbb{I} & \mathrm{tanh}\left(\frac{{\boldsymbol{B}^{(1)}_{\eta} } }{2} \right) &   &  \vdots \\
			\vdots &   &   \ddots & \vdots \\
			\mathbb{I} & \dots   & \dots & \mathrm{tanh}\left(\frac{{\boldsymbol{B}^{(N_\tau)}_{\eta} }}{2} \right) \\
		\end{bmatrix}
		\right) \nonumber \\
		...
		\begin{bmatrix}
			\mathrm{tanh}\left(\frac{{\boldsymbol{A}_{\gamma}  }}{2} \right)    & -\mathbb{I} &  \dots &  -\mathbb{I} \\
			\mathbb{I} & \mathrm{tanh}\left(\frac{{\pm\boldsymbol{B}^{(1)}_{\gamma} } }{2} \right) &   &  \vdots \\
			\vdots &   &   \ddots & \vdots \\
			\mathbb{I} & \dots   & \dots & \mathrm{tanh}\left(\frac{{\pm\boldsymbol{B}^{(N_\tau)}_{\gamma} }}{2} \right) \\
		\end{bmatrix}
		\right),
	\end{align}
