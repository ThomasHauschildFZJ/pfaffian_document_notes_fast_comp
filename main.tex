...
\end{align}
 ...
 For HMC we require the real part of the gradient of the action. One possibility is to take the gradient of the caterpillar and work with its real part. However, there is a more efficient version which relies on the fact, that $W = \pm \sqrt{\mathrm{det}\left(  M \right)}$, where $M = \mathbb{I} + B_1 ... B_{2N_{\tau} }$. We only need the real part of the gradient of the action, which is the same as taking the derivative of the absolute value of the weight:
 ...
 and the absolute value of the weight is $|W[\phi]| = +\sqrt{\mathrm{det} \left(\mathbb{I} + B_1 ... B_{2N_\tau} \right) }$. So it suffices to take the derivative of the square root of the determinant as opposed to the more complicated weight involving pfaffians. We then have
...
where $\mathrm{Tr} _{f}$ is to be understood as a trace over the block matrices only. After performing the trace there are only spacial indices left. Let us take a closer look at the result. After re substituting $R^{k}$ we find
...
\begin{align}
	\frac{\mathrm{d} }{\mathrm{d}\phi^{k}_{x} }\left(-\mathrm{log} \left(|W[\phi]|\right)   \right) &= \mathrm{Re} \left[\xi \mathrm{Tr} _{f} \left\{ \begin{bmatrix}
		0 & +\mathbb{I} \\
		-\mathbb{I} & 0 \\
	\end{bmatrix}   \mathcal{K} B_{2k+2}\mathcal{K} B_{2k + 4}...\mathcal{K} B_{2N_\tau}M^{-1}\mathcal{K} B_{2}...\mathcal{K} B_{2k-2}\mathcal{K}\right\}_{x,x} \right] \\
	&= \mathrm{Re} \left[\xi \mathrm{Tr} _{f} \left\{ \begin{bmatrix}
		0 & +\mathbb{I} \\
		-\mathbb{I} & 0 \\
	...
	&=  \mathrm{Re}\left(  \xi \mathrm{Tr} _{f} \left\{  \begin{bmatrix}
		0 & +\mathbb{I} \\
		-\mathbb{I} & 0 \\
	\end{bmatrix} \mathcal{B}\left(k\right)M^{-1} \mathcal{A}\left(k-1\right)    \right\}_{x,x}\right)\\
	&= \mathrm{Re}\left(  \xi  \left(\mathcal{B}\left(k\right)M^{-1} \mathcal{A}\left(k-1\right)_{2,1} - \mathcal{B}\left(k\right)M^{-1} \mathcal{A}\left(k-1\right)_{1,2}  \right)_{x,x}  \right)
\end{align} 



Here we used the fact that $J$ and $B_{2k}$ commute with one another and the cyclicity of the trace (careful, this is not really rigorous because it is not the full trace, however, it can be shown to be true by defining the x-projection operator and promoting the trace to a full trace and then use cyclicity). This result is very similar to the result one finds in DQMC, where one usually assumes a diagonal interaction. It should be noted here however, that the definitions of $\mathcal{A}(k)$ and $\mathcal{B}(k)$ are slightly different from Finns notes.
