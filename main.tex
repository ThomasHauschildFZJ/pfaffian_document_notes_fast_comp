...
\end{align}
 ...
 For HMC we require the real part of the gradient of the action. One possibility is to take the gradient of the caterpillar and work with its real part. However, there is a more efficient version which relies on the fact, that $W = \pm \sqrt{M}$, where $M = \mathbb{I} + B_1 ... B_{2N_{\tau} }$. We only need the real part of the gradient of the action, which is the same as taking the derivative of the absolute value of the weight:
 ...
 \begin{align}
 	...
 	&= -\frac{1}{2}\frac{1}{|W[\phi]|}\mathrm{Re}\left(   \frac{\mathrm{d} }{\mathrm{d}\phi_{x}^{k}} \mathrm{det}\left(\mathbb{I} + B_1...B_{2N_\tau}\right)\right) \\
...
\begin{align}
	\left[ \frac{\mathrm{d} }{\mathrm{d}\phi^{k}_{x} } B_{2k}  \right]_{(l',f')(l'',f'')} = -2\xi \delta_{l'x}\delta_{l''x}\left[ s^{k}_{x} \left(\delta_{f'1}\delta_{f''1} + \delta_{f'2}\delta_{f''2}\right) + c^{t}_{x}\left(\delta_{f'1}\delta_{f''2} - \delta_{f'2}\delta_{f''1}  \right) \right]    .
\end{align}

Next we look at the product (with Einstein sum convention)

\begin{align}
	\mathcal{K}_{(l,f)(l',f')} \cdot \left[ \frac{\mathrm{d} }{\mathrm{d}\phi^{k}_{x} } B_{2k}  \right]_{(l',f')(l'',f'')} &= -2\xi  \mathcal{K}_{(l,f)(l',f')} \delta_{l'x}\delta_{l''x}\left[ s^{k}_{x} \left(\delta_{f'1}\delta_{f''1} + \delta_{f'2}\delta_{f''2}\right) + c^{t}_{x}\left(\delta_{f'1}\delta_{f''2} - \delta_{f'2}\delta_{f''1}  \right) \right]\\
	&= -2\xi \delta_{l''x}\left(\mathcal{K}_{(l,f)(x,1)}s^{t}_{x} \delta_{f''1} + \mathcal{K}_{(l,f)(x,2)}s^{t}_{x} \delta_{f''2} 
	+\mathcal{K}_{(l,f)(x,1)}c^{t}_{x} \delta_{f''2} - \mathcal{K}_{(l,f)(x,2)}c^{t}_{x} \delta_{f''1} \right)  
\end{align}
...
\begin{align}
	...
	&= \mathrm{Re}\left(\xi \delta_{l''x}\left(\mathcal{K}_{(l,f)(x,1)}s^{t}_{x} \delta_{f''1} + \mathcal{K}_{(l,f)(x,2)}s^{t}_{x} \delta_{f''2} 
	+\mathcal{K}_{(l,f)(x,1)}c^{t}_{x} \delta_{f''2} - \mathcal{K}_{(l,f)(x,2)}c^{t}_{x} \delta_{f''1} \right)  R^{k}_{(l'',f'')(l,f)}\right)\nonumber \\
	&= \mathrm{Re}\left(\xi \left(\mathcal{K}_{(l,f)(x,1)}R^{k}_{(x,1)(l,f)}s^{t}_{x} + \mathcal{K}_{(l,f)(x,2)}R^{k}_{(x,2)(l,f)}s^{t}_{x} + \mathcal{K}_{(l,f)(x,1)}R^{k}_{(x,2)(l,f)}c^{t}_{x} - \mathcal{K}_{(l,f)(x,2)}R^{k}_{(x,1)(l,f)}c^{t}_{x} \right)\right)\nonumber\\
	&= \mathrm{Re}\left(\xi \left[ \left(R\mathcal{K}\right)_{(x,1)(x,1)} s^{t}_{x}  + \left(R\mathcal{K}\right)_{(x,2)(x,2)} s^{t}_{x} + \left(R\mathcal{K}\right)_{(x,2)(x,1)} c^{t}_{x} - \left(R\mathcal{K}\right)_{(x,1)(x,2)} c^{t}_{x}       \right]\right)\nonumber \\
	&= \mathrm{Re}\left(\xi \mathrm{Tr}_{f}\left\{\begin{bmatrix}
		D\left(s^{t} \right) & D\left(c^{t} \right) \\
		-D\left(c^{t} \right) & D\left(s^{t} \right) 
...
\begin{align}
	\frac{\mathrm{d} }{\mathrm{d}\phi^{k}_{x} }\left(-\mathrm{log} \left(|W[\phi]|\right)   \right) &= \xi \mathrm{Tr} _{f} \left\{ \begin{bmatrix}
		0 & +\mathbb{I} \\
		-\mathbb{I} & 0 \\
	\end{bmatrix}   \underbrace{B_{2k} \mathcal{K} B_{2k+2}\mathcal{K} B_{2k + 4}...\mathcal{K} B_{2N_\tau}}_{:=\mathcal{B}(k)}M^{-1}\underbrace{\mathcal{K} B_{2}...\mathcal{K} B_{2k-2}\mathcal{K}}_{:= \mathcal{A}(k)}  \right\}_{x,x}  \\
	&= \xi \mathrm{Tr} _{f} \left\{   \mathcal{B}\left(k\right)M^{-1} \mathcal{A}\left(k-1\right)   \begin{bmatrix}
		0 & +\mathbb{I} \\
		-\mathbb{I} & 0 \\
	\end{bmatrix} \right\}_{x,x}\\
	&= \xi  \left(\mathcal{B}\left(k\right)M^{-1} \mathcal{A}\left(k-1\right)_{1,2} - \mathcal{B}\left(k\right)M^{-1} \mathcal{A}\left(k-1\right)_{2,1}  \right)_{x,x}  
\end{align} 



This result is very similar to the result one finds in DQMC, where one usually assumes a diagonal interaction.
